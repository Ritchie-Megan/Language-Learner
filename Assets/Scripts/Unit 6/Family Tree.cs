...
public class FamilyTree : MonoBehaviour
{
    ...
    {
        int maxPeople = 10;
        Node root = buildTree(maxPeople);
        Debug.Log("-------Family Tree----------");
        HashSet<Node> visitedNodes = new HashSet<Node>();
        List<String> cluesList = new List<String>();
        generateAndPrintTreeAndClues(root, visitedNodes, ref cluesList);
        Debug.Log("Number of nodes in hashset: " + visitedNodes.Count);
        foreach (Node person in visitedNodes) {
            Debug.Log(person.personName);
        }
        /*
        //connect to UI
        FamilyTreeUI ui = FindObjectOfType<FamilyTreeUI>();
        if (ui != null) {
            ui.GenerateTextNodes(visitedNodes);
        }
        */
    }
    ...
    public class Node {
        ...
        //m = married, s = single, d = divorce, r = remarried
    ...
    Node buildTree(int maxNumberOfPeople) {
    ...
    void generateNodeInfo(Node currentNode, ref int numPeopleLeft, ref List<Node> toVisitQueue) {
    ...
    void generateAndPrintTreeAndClues(Node root, HashSet<Node> visited, ref List<string> clues, int depth = 0) {
        //base case
        if (root == null) {
            return;
        }
    ...
    //generate the clues
    void generateFamilyRelationClue(Node currentNode, ref List<string> clues) {
